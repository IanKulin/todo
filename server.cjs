...
app.post("/todos", (req, res) => {
  if (!req.body.todo_item) {
    return res
      .status(400)
      .json({ error: "Missing todo_item field in request body" });
  }
  db.run(
    "INSERT INTO todos (todo_item) VALUES (?)",
    req.body.todo_item,
    function (err) {
      if (err) {
...
db.run(
  "CREATE TABLE IF NOT EXISTS todos (id INTEGER PRIMARY KEY AUTOINCREMENT, todo_item TEXT)"
);
